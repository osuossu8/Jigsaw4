...
stop_words = ['i', 'me', 'my', 'myself', 'we', 'our', 'ours', 'ourselves', 
              ...
              "i'm", "i've", "i'd", 'it', "i'll", 'i;m', 'would', 'could']


def clean_stopwords(x_list):
    res = []    
    for x in x_list:
        if x in stop_words:
            continue
...
train['text'] = train['text'].map(lambda x: clean_stopwords(x.lower().replace('\n', ' ').replace('\t', ' ').replace('.', ' ').replace(',', ' ').replace('"', ' ').replace("'s", ' ').replace('?', ' ').replace('!', ' ').split()))
...
def calc_cv(model_paths):
    ...
    df_more['text'] = df_more['text'].map(lambda x: clean_stopwords(x.lower().replace('\n', ' ').replace('\t', ' ').replace('.', ' ').replace(',', ' ').replace('"', ' ').replace("'s", ' ').replace('?', ' ').replace('!', ' ').split()))
    df_less['text'] = df_less['text'].map(lambda x: clean_stopwords(x.lower().replace('\n', ' ').replace('\t', ' ').replace('.', ' ').replace(',', ' ').replace('"', ' ').replace("'s", ' ').replace('?', ' ').replace('!', ' ').split()))

    df_more['text'] = df_more['text'].map(text_cleaning)
    df_less['text'] = df_less['text'].map(text_cleaning)
