...
train_ruddit = pd.read_csv('input/RudditJigsaw/Dataset/jigsaw_ruddit_toxic_score.csv')
print(train_ruddit.shape)
train_ruddit = train_ruddit[train_ruddit['text']!='[deleted]'].reset_index(drop=True)
train_ruddit = train_ruddit[train_ruddit['y']>0.2].reset_index(drop=True)
print(train_ruddit.shape)
...
train_ruddit['text'] = train_ruddit['text'].map(text_cleaning)
...
for n, (trn_index, val_index) in enumerate(Fold.split(train_ruddit, train_ruddit['y'])):
    train_ruddit.loc[val_index, 'kfold'] = int(n)
train_ruddit['kfold'] = train_ruddit['kfold'].astype(int)


train = pd.concat([train, train_ruddit], 0).reset_index(drop=True)
print(train.shape)
