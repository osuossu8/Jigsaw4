...
texts = set(train.less_toxic.to_list() + train.more_toxic.to_list())
text2id = {t:id for id,t in enumerate(texts)}
train['less_id'] = train['less_toxic'].map(text2id)
train['more_id'] = train['more_toxic'].map(text2id)
...
train['less_gid'] = train['less_id'].map(id2groupid)
train['more_gid'] = train['more_id'].map(id2groupid)

print('unique text counts:', len_ids)
print('grouped text counts:', len(group_list))

Fold = GroupKFold(n_splits=CFG.N_FOLDS)
for n, (trn_index, val_index) in enumerate(Fold.split(train, train, train['less_gid'])):
    train.loc[val_index, 'kfold'] = int(n)
train['kfold'] = train['kfold'].astype(int)


train.to_csv('input/train_folds_tito.csv', index=False)
...
def calc_cv(model_paths):
    ...
    df = pd.read_csv("input/train_folds_tito.csv")
