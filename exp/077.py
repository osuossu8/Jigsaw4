...
external = external.sample(n=60000, random_state=CFG.seed).reset_index(drop=True)
print(external.shape)
...
for fold in range(5):
    ...
    trn_df = external[external.kfold != fold].reset_index(drop=True)
    val_df = train.copy().reset_index(drop=True)    
