...
# Data Loading
# ====================================================
external_path = 'input/ExternalAll/'

# train = pd.read_csv('input/validation_data.csv')
j1_df = pd.read_csv(external_path+'jig1_no_jig4_dup_df.csv')
j1_df['severe_toxic'] = j1_df.severe_toxic * 2
j1_df['y'] = (j1_df[['toxic', 'severe_toxic', 'obscene', 'threat', 'insult', 'identity_hate']].sum(axis=1) ).astype(int)
j1_df = j1_df[['text', 'y']]

good_df = pd.read_csv(external_path+'good_no_jig4_dup_df.csv')[['text', 'y']]
...
train = pd.concat([j1_df, good_df]).reset_index(drop=True)
