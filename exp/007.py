...
external = pd.read_csv('input/JigsawUnintendedBias/external_data_jigsaw_unintended_bias.csv')
external = external[external['worker']>752].reset_index(drop=True)
print(external.shape)

Fold = GroupKFold(n_splits=CFG.N_FOLDS)
for n, (trn_index, val_index) in enumerate(Fold.split(external, external, external['worker'])):
    external.loc[val_index, 'kfold'] = int(n)
external['kfold'] = external['kfold'].astype(int)

external.to_csv('input/external_folds.csv', index=False)
