...
df = pd.concat([df_train, df_test]).rename(columns={'comment_text': 'text'})
...
train = pd.merge(train, df, on=['text'])
print(train.shape)
print(train.columns)
print(train.head())
...
class Jigsaw4Dataset:
    def __init__(self, df, cfg):
        ...
        if 'y' in df.columns:
            ...
            self.target = df[cfg.target_cols].values
        else:
            self.target = None
    ...
    def __getitem__(self, item):
        ...
        if self.target is None:
            ...
            targets = torch.tensor(6, dtype=torch.float)
        else:
...
class RoBERTaBase(nn.Module):
    def __init__(self, model_path):
        ...
        self.l0 = nn.Linear(self.in_features, 6)
